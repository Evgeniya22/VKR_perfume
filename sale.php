...
<form name="form2"  method="post"  >
   ...
   if ($step==1)
	...
	$Arr=$_POST['Arrsale'];
	$idsale=$Arr[0];
	mysqli_query($dbcnx,"UPDATE sale SET status='Продано' where idsale=$idsale");

	}
	//echo "UPDATE sale SET status='Продано' where idsale=$idsale";
...
$s="SELECT datesale, datedelivery, status,  CONCAT (surname,' ', firstname,' ', patronymic) as fio,  (select SUM(price*countmerch) from detail, merch where detail.idsale=sale.idsale and detail.idmerch=merch.idmerch) as sumsale  FROM sale  INNER JOIN user ON user.iduser=sale.iduser where fio" ;
if ($value1!="Все")
$s=$s." LIKE '%$value1"."%'";
Else
$s=$s."=fio ";
		 ...
		 $s="SELECT idsale, datesale, datedelivery,  status,  CONCAT (surname,' ', firstname,' ', patronymic) as fio,  (select SUM(price*countmerch) from detail, merch where detail.idsale=sale.idsale and detail.idmerch=merch.idmerch) as sumsale  FROM sale INNER JOIN user ON user.iduser=sale.iduser";
...
Сортировка:
				<select name="sortname"  style="height:22; width:auto" onChange="this.form.action='sale.php?sort=1&filter=<? echo $filter;?>'; this.form.submit();" >
					...
					<option value="status"  <? if ($fieldsort=="status") {?> selected="selected" <? }?>>Категория </option>	
					<option value="fio"  <? if ($fieldsort=="fio") {?> selected="selected" <? }?>>Покупатель</option>	
 					<option value="sumsale DESC"  <? if ($fieldsort=="sumsale DESC") {?> selected="selected" <? }?>>Сумма </option>	                                      
				</select>            	
ФИО покупателя: 
